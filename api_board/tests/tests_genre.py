...
class TestGenreViewSet(TestCase):
    ...
    def test_get_genre_list_for_not_auth_user(self):
        # call_command('loaddata', 'api_board/tests/genrefixture.json', verbosity=0)
        response = self.not_auth_client.get(self.list_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_get_genre_list_pagination(self):
        response = self.not_auth_client.get(self.list_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(response.data.get('count'), Genre.objects.count())

    def test_get_genre_list_response_data(self):
    ...
    def test_create_genre_without_slug(self):
        ...
        self.assertEqual(response.data['slug'], genre.slug)
    ...
    def test_filter_by_genre_name(self):
        ...
        queryset = Genre.objects.filter(name__icontains=param)
    ...
    def test_delete_genre_by_admin(self):
        ...
        with self.assertRaises(exceptions.ObjectDoesNotExist):
            Genre.objects.get(slug=self.slug)
