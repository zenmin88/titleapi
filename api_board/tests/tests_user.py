...
class TestUser(TestCase):
    ...
    def test_patch_current_user_field_role(self):
        """Only admin can change role"""
        data = {'role': 'moderator'}
        response = self.user_client.patch(self.current_user_url, data=data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        response = self.admin_client.patch(self.current_user_url, data=data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_patch_current_user_invalid_data(self):
        response = self.user_client.patch(self.current_user_url, data={'username': ''})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def check_response_data(self, data, user):
        """
        Compare request and User objects.
        """
